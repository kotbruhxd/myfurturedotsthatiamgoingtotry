{
  ...
  outputs = { self, nixpkgs, home-manager, quickshell, catppuccin, nur, lanzaboote, spicetify-nix }:
    let
      ...
      runtimeDeps = pkgs:
        with pkgs; [
          bash bc coreutils curl findutils gawk git gnugrep gnused jq
          procps python3 ripgrep rsync systemd wget xdg-utils
          quickshell wl-clipboard cliphist grim slurp playerctl
          libnotify glib pipewire pulseaudio wireplumber
        ]
          ...
          options.programs.inir = {
            ...
            service = {
              ...
              compositor = lib.mkOption {
                ...
                default = "niri";
                description = "Compositor user unit that should want inir.service.";
              };
            };
          };
        };
        ...
        in
        {
          ...
          options.programs.inir.configSymlink = {
            enable = lib.mkOption {
              type = lib.types.bool;
              default = false;
              description = "Expose the packaged shell at ~/.config/quickshell/inir.";
            };
          };
          config = lib.mkIf cfg.enable {
